...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const audioDebugger = document.getElementById('audio-debugger');
    ...
    function resumeAllSounds() {
        if (config.music !== 'none' && !audio[config.music].loop) {
            audio[config.music].loop = true;
        }
    ...
    function resetApp() {
        ...
        // Reset pacer dot
        updatePacer(3, 1); // Reset to bottom-left corner
    ...
    function startBreathingSession() {
        ...
        updateTimerDisplay(config.duration);
        updatePacer(3, 1); // Start at bottom-left
    ...
    function updateUI(currentPhase, sideDuration, cycleProgress) {
        ...
        // Update audio debugger
        let debugText = `Music: ${config.music} (${audio[config.music] && !audio[config.music].paused ? 'Playing' : 'Paused'})<br>`;
        debugText += `Phase Sound: ${getPhaseSoundName(currentPhase)} (${!audio[getPhaseSoundName(currentPhase)].paused ? 'Playing' : 'Paused'})<br>`;
        debugText += `Instructions: ${getInstructionSoundName(currentPhase)} (${config.instructions ? 'Enabled' : 'Disabled'}, ${!audio[getInstructionSoundName(currentPhase)].paused ? 'Playing' : 'Paused'})`;
        audioDebugger.innerHTML = debugText;
    }

    function playPhaseSounds(phase) {
        resetPhaseSounds();
        if (phase === 0) {
            audio['phase-inhale'].play().catch(e => console.error('phase-inhale failed', e));
            if (config.instructions) audio['instruction-inhale'].play().catch(e => console.error('instruction-inhale failed', e));
        } else if (phase === 1 || phase === 3) {
            audio['phase-hold'].play().catch(e => console.error('phase-hold failed', e));
            if (config.instructions) audio['instruction-hold'].play().catch(e => console.error('instruction-hold failed', e));
        } else if (phase === 2) {
            audio['phase-exhale'].play().catch(e => console.error('phase-exhale failed', e));
            if (config.instructions) audio['instruction-exhale'].play().catch(e => console.error('instruction-exhale failed', e));
        }
    }

    function getPhaseSoundName(phase) {
        if (phase === 0) return 'phase-inhale';
        if (phase === 1 || phase === 3) return 'phase-hold';
        if (phase === 2) return 'phase-exhale';
    }

    function getInstructionSoundName(phase) {
        if (phase === 0) return 'instruction-inhale';
        if (phase === 1 || phase === 3) return 'instruction-hold';
        if (phase === 2) return 'instruction-exhale';
    }
